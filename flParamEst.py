...
if __name__ == "__main__":    
    ...
    param = ModParam()
    if param.mcmc:
        param.gr_fp = np.random.uniform(0, 7000, (param.n_samples,cbParam.n_reactors)).T
        param.e1_ofs = np.random.uniform(0, [cbData.fl[r][0]*cbData.b1[r][0] for r in range(cbParam.n_reactors)], (param.n_samples,cbParam.n_reactors)).T
        param.od_fac = np.random.uniform(0, 1000, (param.n_samples)).T
        # param.od_ofs = np.random.uniform(0.05, 0.3, (n_samples))

    # SIMULATE
    sim_hrs, cb_dil, fp_init, sim_puti = [], [], [], []
    sim_e1_train, sim_e1_test = [], []
    ...
    if param.mcmc:
        for j in range(cbParam.n_reactors):
            ...
            sim_e1_train.append((sim_fp.T + param.e1_ofs[j] + np.outer(cbData.od[j],param.od_fac)).T)
            rmse = np.sqrt(np.mean((sim_e1_train[j] - cbData.fl[j]*cbData.b1[j])**2,axis=1))
            s_min[j] = np.argmin(rmse)

        param.gr_fp = np.array([param.gr_fp[j][s_min[j]] for j in range(cbParam.n_reactors)])
        param.e1_ofs = np.array([param.e1_ofs[j][s_min[j]] for j in range(cbParam.n_reactors)])
        param.od_fac = np.array([param.od_fac[s_min[j]] for j in range(cbParam.n_reactors)])
        print("Growth rates:")
        print(*param.gr_fp, sep = ", ")
        print("e1 ofs:")
        print(*param.e1_ofs, sep = ", ")
        print("od fac:")
        print(*param.od_fac, sep = ", ")
        # print("od_ofs:")
    ...
    # for j in range(cbParam.n_reactors):
    #     fl_init_j = (cbData.fl[j][0] - param.e1_ofs[j])*(cbData.od[j][0]+param.od_ofs)
    #     sim_fp = simulateFlProtein(fl_init_j, cbData.time_h[j], sim_hrs[j], cbData.temp[j], sim_puti[j], cb_dil[j], j, param).T
    #     sim_e1_test.append(((sim_fp/(cbData.od[j]+np.full((len(cbData.od[j]),param.n_samples),param.od_ofs).T)).T+param.e1_ofs[j]).T)
    # param.mcmc = mcmc
    ...
    for j in range(cbParam.n_reactors):
        ...
        if param.mcmc:
            ax[r][c].plot(cbData.time_h[j],sim_e1_train[j][0],'k',lw = 0.5, label = '$e1_{sim, train}$', alpha = 0.1)
            for s in range(1,min(param.n_samples,100)):
                ax[r][c].plot(cbData.time_h[j],sim_e1_train[j][s],'k',lw = 0.5, alpha = 0.1)
            ax[r][c].plot(cbData.time_h[j],sim_e1_train[j][s_min[j]],'m', lw = 0.7, label = '$e1_{sim, opt}$')
        # ax[r][c].plot(cbData.time_h[j],sim_e1_test[j][0],'b',lw = 0.7, label = '$fl_{sim, model}$')
        ...
        ax[r][c].set_ylim([0,1.1*max(cbData.fl[j]*cbData.b1[j])])
        axr.set_ylim([25,38])
    ...
    fig.savefig(results_dir + "/fl_sim_e1_{}.png".format("train" if param.mcmc else "test"))